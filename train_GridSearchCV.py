...
grid = {"n_estimators":np.arange(50,201,10), "max_features":np.arange(1, 167, 1), "criterion":["gini", "entropy"]}
...
svm_clf_cv = GridSearchCV(svm_clf,grid, cv=10)
...
svm_clf_cv.fit(train_data, train_label.ravel())
print("Optimal Hyperparameters: ", svm_clf_cv.best_params_)
#print("Train Score: ", svm_clf_cv.best_score_)



#使用网格搜索方法进行参数寻优
#对n_estimators参数择优，其他参数仍然是默认值
#param_test1={'n_estimators':range(1,201,10)}
#grid_search_1=GridSearchCV(estimator=RandomForestClassifier(random_state=42),param_grid=param_test1,scoring='roc_auc',cv=10)
#grid_search_1.fit(train_data,train_label.ravel())
#print(grid_search_1.best_params_)
#print(grid_search_1.best_score_)

#max_features参数择优，n_estimators参数设置为 其它参数为默认值
#param_test2={'max_features':range(1,21,1)}
#grid_search_2=GridSearchCV(estimator=RandomForestClassifier(n_estimators=grid_search_1.best_params_['n_estimators'],random_state=42),param_grid=param_test2,scoring='roc_auc',cv=10)
#grid_search_2.fit(train_data,train_label.ravel())
#print(grid_search_2.best_params_)
#print(grid_search_2.best_score_)

#寻优结束，用最优参数重新进行训练
#svm_clf=RandomForestClassifier(n_estimators=grid_search_1.best_params_['n_estimators'],max_features=grid_search_2.best_params_['max_features'],oob_score=True,random_state=42)
...
pickle.dump(svm_clf_cv, file)
file.close()
train_score = svm_clf_cv.score(train_data,train_label)
print("训练集：", train_score)
test_score = svm_clf_cv.score(test_data,test_label)
print("测试集：", test_score)
y_hat = svm_clf_cv.predict(test_data)
